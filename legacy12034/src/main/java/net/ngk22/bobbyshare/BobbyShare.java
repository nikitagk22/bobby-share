...
public class BobbyShare implements ModInitializer {
    ...
    @Override public void onInitialize() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(ChunkRequestPayload.ID, (server, player, handler, buf, responseSender) -> {
            int x = buf.readInt();
            int z = buf.readInt();
            STATS_TOTAL_REQUESTS.incrementAndGet();
            server.execute(() -> serve(server, player, new ChunkPos(x, z)));
        });
    }
                ...
                for (ServerPlayerEntity player : world.getPlayers()) {
                    if (!player.isDisconnected() && ServerPlayNetworking.canSend(player, ChunkInvalidationPayload.ID)) {
                        ...
                        if (dx * dx + dz * dz <= maxDistSq) {
                            ...
                            ServerPlayNetworking.send(player, ChunkInvalidationPayload.ID, out);
    ...
    private static void send(ServerPlayerEntity player, ChunkPos pos, Optional<NbtCompound> nbt) {
        PacketByteBuf out = new PacketByteBuf(io.netty.buffer.Unpooled.buffer());
        out.writeInt(pos.x); out.writeInt(pos.z); out.writeBoolean(nbt.isPresent());
        nbt.ifPresent(out::writeNbt);
        ServerPlayNetworking.send(player, ChunkResponsePayload.ID, out);
    }
    ...
    private static NbtCompound optimize(NbtCompound tag) {
        ...
        if (tag.contains("block_entities", 9)) {
            net.minecraft.nbt.NbtList list = tag.getList("block_entities", 10);
