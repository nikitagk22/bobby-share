...
public class BobbyShare implements ModInitializer {
    ...
    private static void serve(net.minecraft.server.MinecraftServer server, ServerPlayerEntity player, ChunkPos pos) {
        ...
        if (liveChunk != null) {
            try {
                Optional<NbtCompound> result = Optional.of(optimize(SerializedChunk.fromChunk(world, liveChunk).serialize()));
        ...
        storage.readChunkData(pos).thenAccept(opt -> {
            ...
            if (player.getEntityWorld() == world && !player.isDisconnected()) send(player, pos, result);
        }).exceptionally(error -> { LOGGER.error("Failed to read chunk " + pos, error); if (player.getEntityWorld() == world && !player.isDisconnected()) send(player, pos, Optional.empty()); return null; });
    }
